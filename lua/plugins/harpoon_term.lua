local function open_term_in_split(term_number)
	...
	local specified_split_height =
		math.floor(current_window_height * (1 - (desired_split_height_percentage / 100)))
...
return {
	...
	keys = {
		...
		},
		{
			"<leader>t1",
			function()
				require("harpoon"):list("term"):select(1)
			end,
		...
		},
		{
			"<leader>t2",
			function()
				require("harpoon"):list("term"):select(2)
			end,
		...
		},
		{
			"<leader>t3",
			function()
				require("harpoon"):list("term"):select(3)
			end,
		...
		},
		{
			"<leader>t4",
			function()
				require("harpoon"):list("term"):select(4)
			end,
